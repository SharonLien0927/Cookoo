<template>
  <div
    @click="handleClick"
    :class="[
      'bg-white rounded-3xl shadow-md border-2 overflow-hidden cursor-pointer transition-all hover:shadow-lg active:scale-[0.98]',
      selected ? 'border-red-500 ring-4 ring-red-500' : 'border-orange-200'
    ]"
  >
    <div class="relative">
      <img
...
const props = withDefaults(defineProps<{ recipe: Recipe; selected?: boolean; onClick?: () => void }>(), { selected: false })
...
const handleFavorite = () => recipeStore.toggleFavorite(props.recipe.id)
